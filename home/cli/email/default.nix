...
with lib; let
  cfg = config.email;
  hasSecretsEnabled = osConfig.secrets.enable && config.secrets.enable;
in {
  options = {
    email = {
      enable = mkEnableOption "Enable E-Mail support using neomutt";
    };
  };
  config = mkIf (hasSecretsEnabled && cfg.enable) {
    home = {
      packages = with pkgs; [
        abook
        lynx
